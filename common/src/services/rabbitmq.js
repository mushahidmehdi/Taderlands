...
const { RABBITMQ_PASS, RABBITMQ_USER, RABBITMQ_HOST, RABBITMQ_PORT } = process.env;

const EventEmitter = require("events");

const mqConnectionEmitter = new EventEmitter();

const connection = amqp.connect([`amqp://${RABBITMQ_USER}:${RABBITMQ_PASS}@${RABBITMQ_HOST}:${RABBITMQ_PORT}`]);
...
const publishMessage = async (channelName, data) => {
	try {
		if (!channelWrapper?.[channelName]) {
			console.log("No channel wrapper for this channel name.");
			return;
		}

		const { wrapper, exchange, routingKey } = await channelWrapper[channelName];

		await wrapper.publish(exchange, routingKey, data, { contentType: "application/json", persistent: true });
	} catch (e) {
		console.log(e);
	}
};

module.exports = { mqConnectionEmitter, createChannelWrapper, publishMessage };
